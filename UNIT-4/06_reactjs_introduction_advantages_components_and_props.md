...
1. **Virtual DOM Engine**:
   ...
   - When component state changes, React compares the new Virtual DOM with a pre-update snapshot using its high-speed **Reconciliation (Diffing) Algorithm** and batches _only_ the specific changed nodes to the real browser DOM.
...
3. **Declarative UI Programming**: Developers describe _what_ the UI should look like for a given state, and React handles rendering and DOM updates automatically.
...
```mermaid
flowchart LR
    Parent["Parent Component (App)"]
    Child["Child Component (UserCard)"]
    UI["Rendered HTML View"]

    Parent -->|Passes: name=Alex, role=Admin| Child
    Child -->|Uses name property| UI
```
